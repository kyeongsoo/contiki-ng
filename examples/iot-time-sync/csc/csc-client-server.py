...
if __name__ == "__main__":
    ...
    df_c = pd.read_csv(client_log_file, header=0)
    server_log_file = arg.server_log_file
    df_s = pd.read_csv(server_log_file, header=0)
    ...
    md_file = Path(client_log_file).with_suffix("").with_suffix(".md")
    ...
    ymin = min(df['ds_err'].min(), df['sp_err'].min())
    ymax = max(df['ds_err'].max(), df['sp_err'].max())
    ax = df.plot(kind='scatter', x='elapsed_second', y='ds_err', color='blue',
            xlabel='Time [s]', ylabel='Event Time Estimation Error [s]',
            label=alg_names['ds'], grid=True, legend=True, figsize=(10, 6),
            xlim=(xmin, xmax), ylim=(ymin, ymax))
    df.plot(kind='scatter', x='elapsed_second', y='sp_err', color='red',
        xlabel='Time [s]', ylabel='Event Time Estimation Error [s]',
