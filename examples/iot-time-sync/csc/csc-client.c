  ...
  const linkaddr_t *src, const linkaddr_t *dest)
{
  ...
  if(len == sizeof(csc_data_t)) {
    ...
    if (cfr_initialized == false) {
      ...
      // LOG_INFO("CSC: num_events=%d, i=%lld, csc_ds=%lld, csc_sp_div=%lld, csc_dp_div=%lld\n",
      //   num_events, elapsed_time, csc_ds(elapsed_time, D, A, &num_iter), csc_sp_div(elapsed_time, D, A, &num_iter), csc_dp_div(elapsed_time, D, A, &num_iter));
